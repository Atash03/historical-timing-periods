...
import { getClampedRadius } from '../lib';
import type { CircleWithDotsProps } from '../types';
import { Circle } from './circle';
import scss from './circle-with-dots.module.scss';

export const CircleWithDots = ({ numberOfDots }: CircleWithDotsProps) => {
  const { activeIndex, activeValue, goToYear } = useData();
    ...
    dots.push(
      ...
      >
        {(i === activeIndex || i === hoveredIndex) && (
          <span className={scss.dotIndex}>{i + 1}</span>
        )}
